...
void consumer(uint32_t id, Fifo *f)
{
    while (true)
    {
        ...
        if (item.id == TERMINATOR_ID && item.v1 == 0 && item.v2 == 0) {
            printf("Consumer %u received terminator and is exiting.\n", id);
            break;
        }
...
int main (int argc, char *argv[])
{
    ...
    for (uint32_t i = 0; i < nc; i++)
    {
        Item terminator = {TERMINATOR_ID, 0, 0};
        fifoInsert(theFifo, terminator);
    }
