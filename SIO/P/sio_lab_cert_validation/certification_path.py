...
def get_issuer_cert(cert: x509.Certificate) -> x509.Certificate:
    ...
    access_desc_it = cert.extensions.get_extension_for_class(x509.AuthorityInformationAccess).value
    
    for access in access_desc_it:
        if access.access_method == oid.AuthorityInformationAccessOID.CA_ISSUERS:
            url = access.access_location.value
            
            response = requests.get(url)
            cert_enc = response.content

            if b'-----BEGIN CERTIFICATE-----' in cert_enc:
                return x509.load_pem_x509_certificate(cert_enc)
            else:
                return x509.load_der_x509_certificate(cert.enc)
...
def build_cert_path(cert: x509.Certificate, trusted_path: str = "/etc/ssl/certs") -> List[x509.Certificate]:
    ...
    while(current_cert.issuer != current_cert.subject):
        ...
        if current_cert.issuer in trusted_certs:
            current_cert = trusted_certs[current_cert.issuer]
